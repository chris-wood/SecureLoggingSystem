...
class KeyManager:
	...
	def __init__(self):
		...
		if (os.path.isfile('pubkey.pkl') and os.path.isfile('masterkey.pkl')):
			print("Loading the master and public key from the file")
			self.public = bytesToObject(pickle.load(open('pubkey.pkl', 'rb')), self.groupObj)
			self.master = bytesToObject(pickle.load(open('masterkey.pkl', 'rb')), self.groupObj)
		else:
			self.cpabe = CPabe_BSW07(self.groupObj)
			(self.public, self.master) = self.cpabe.setup()

			# Persist the keys
			outputPublic = open('pubkey.pkl', 'wb')
			outputMaster = open('masterkey.pkl', 'wb')
